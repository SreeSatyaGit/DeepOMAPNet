...
def read_dem_file(filepath):
    ...
    with gzip.open(filepath, 'rt') as f:
        ...
        for line in f:
            ...
            if len(parts) == 3:  # gene_idx, cell_idx, count
                data.append([int(parts[0])-1, int(parts[1])-1, float(parts[2])])  # Convert to 0-based indexing
...
def extract_sample_info(filename):
    ...
    """
    # Remove .dem.txt.gz or .anno.txt.gz
    base_name = filename.replace('.dem.txt.gz', '').replace('.anno.txt.gz', '')
    
    # Parse GSM ID and sample info
    ...
    if len(parts) > 1:
        ...
        # Parse sample type and timepoint
...
def process_gsm_files(data_dir):
    ...
    for i, dem_file in enumerate(dem_files):
        ...
        # Ensure annotation has the right number of cells
        ...
        # Add sample information to annotations
    ...
    # Create gene names (assuming genes are consistent across samples)
    # For now, create generic gene names
    ...
    adata = ad.AnnData(
        X=combined_X.T,  # Transpose to get cells x genes
        obs=combined_obs,
        var=var_df
    )
